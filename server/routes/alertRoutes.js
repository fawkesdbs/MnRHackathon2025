  ...
  try {
    ...
    const result = await pool
      ...
      .input("severityLevel", sql.Int, criticalSeverityInt).query(`
                ...
                JOIN dbo.user_destinations ud ON a.destination_id = ud.destination_id
                WHERE ud.user_id = @userId 
                AND a.severity = @severityLevel 
    ...
    const result = await pool
      ...
      .input("userId", sql.Int, req.params.userId).query(`
            SELECT 
                ...
                LEFT JOIN dbo.alerts a ON d.id = a.destination_id AND a.created_at >= DATEADD(day, -7, GETDATE())
                WHERE ud.user_id = @userId
